---
name: moloch-agent
description: Operate an autonomous Guild agent on DAOhaus/Moloch V3 with minimal setup using the HausDAO hosted moloch service for Graph reads and IPFS pinning. Use when the operator wants to provide only a Guild, mandate, and local signing wallet instead of managing Graph/Pinata credentials.
---

# Moloch Agent

Use this skill as the low-friction entry point for autonomous Guild agents on Base.

Use "Guild" with operators and in generated task/proposal language. A Guild is the operator-facing name for the underlying DAOhaus/Moloch V3/Baal Guild. Keep DAOhaus, Moloch, Baal, and `dao` terms only when referring to protocol names, legacy command aliases, Graph fields, Poster tags, or contract data.
...
- Platform scheduler/task skill, if available: use it to register recurring Guild checks.
...
- Guild database records: shared communication and memory events.
...
1. Confirm Guild address or summon intent.
...
5. Discover existing shared memory pointers from Guild metadata, or create starter pointers when summoning.
6. Run a task snapshot once a Guild exists.
...
For a new Guild, create a small params file and summon through the npm CLI:

```bash
moloch-agent summon --params summon.json
```

The summon params should include initial members, raw initial share/loot base-unit balances, token names, voting/grace periods, quorum, sponsor threshold, and min retention. If Guild metadata pointers such as `communityMemoryURI`, `proposalWorkspaceURI`, or `sharedStateURI` are omitted, the CLI creates and pins a starter Guild workspace and includes its `ipfs://...` URI in summon metadata.
...
Read Guild state through the npm CLI:

```bash
moloch-agent guild --guild 0xGUILD
moloch-agent guild-url --guild 0xGUILD
moloch-agent links --guild 0xGUILD --proposal 12
moloch-agent read-guild --guild 0xGUILD
moloch-agent balances --guild 0xGUILD
moloch-agent balances --address 0xADDRESS --token 0xERC20
moloch-agent proposals --guild 0xGUILD
moloch-agent proposal-lifecycle --guild 0xGUILD --proposal 12
moloch-agent process-queue --guild 0xGUILD
moloch-agent members --guild 0xGUILD
moloch-agent records --guild 0xGUILD --table communityMemory
```

`guild-url` returns the DAOhaus Admin proposals URL, for example:

```text
https://admin.daohaus.club/molochv3/0x2105/0xGUILD/proposals
```
...
Update Guild metadata pointers through a governance proposal:

```bash
moloch-agent guild-meta \
  --guild 0xGUILD \
  --title "Update Guild memory pointers" \
...
- Post concise Guild database memory records after meaningful actions.
...
1. Read current Guild state and memory.
...
7. Use the correct proposal path:
   ...
   - token tribute / join / swap: `tribute`, `join-guild`, `swap`, or `token-swap`
...
moloch-agent gov-settings --guild 0xGUILD --params gov-settings.json
moloch-agent token-settings --guild 0xGUILD --pause-shares false --pause-loot false
moloch-agent custom-proposal --guild 0xGUILD --title "Custom action" --actions actions.json
...
moloch-agent ragequit --guild 0xGUILD --to 0xRECIPIENT --shares 1 --loot 0 --tokens ETH --confirm-ragequit
moloch-agent join-guild --guild 0xGUILD --token 0xERC20 --amount 1000000 --shares 10000
moloch-agent tribute --guild 0xGUILD --token 0xERC20 --amount 1000000 --shares 10000
...
moloch-agent mint-shares --guild 0xGUILD --to 0xMEMBER --amount 1
...
For `mint-shares` and `mint-loot`, `--amount 1` means 1 full Guild token and encodes as `1000000000000000000`. Use `--amount-raw` only when the exact base-unit value is intended.
...
```bash
moloch-agent process-ready --guild 0xGUILD
```
