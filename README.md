...
- Proposal builders estimate `baalGas` when `RPC_URL` or `--rpc` is configured. The estimator uses the DAO Safe/module path like DAOhaus Admin: wrap MultiSend in `execTransactionFromModule`, estimate from the Baal address, then add `150000` gas per inner action. If estimation fails, the builder falls back to `0` and reports `baalGasEstimateError`; use `--require-baal-gas-estimate` to make that a hard error.
